...
class MesDeTrabajoController extends \BaseController {
    ...
    public function centralizar()
    {
        ...
        if( $empresa->rut=='965799206' ){
            ...
            foreach($comprobante['Detalle'] as $comprobanteDetalle){
                ...
                $comprobanteRRhhDetalle->pais = $comprobanteDetalle['CentroCosto1'];
                $comprobanteRRhhDetalle->canal = $comprobanteDetalle['CentroCosto2'];
                $comprobanteRRhhDetalle->tienda = $comprobanteDetalle['CentroCosto3'];
            ...
            $result = $client->post('http://' . $sub . '.cme-es.com/rest/rrhh/centralizar-libro', [
                'auth' => ['user@interconexion', 'easy1q2w3e4r'],
                'json' => [
            ...
            if( $datosConexionCME['success'] ){
                
                $comprobanteRRhh = new ComprobanteCentralizacion();
                $comprobanteRRhh->fecha = $comprobante['Fecha'];
                $comprobanteRRhh->mes = $mes->mes;
                $comprobanteRRhh->referencia=$comprobante['Referencia'];
                $comprobanteRRhh->empresa = $empresa->razon_social;
                $comprobanteRRhh->comentario = $comprobante['Comentario'];
                $comprobanteRRhh->numero = 1;
                $comprobanteRRhh->save();
                ...
                foreach($comprobante['Detalle'] as $comprobanteDetalle){
                    ...
                    $comprobanteRRhhDetalle->debe = $comprobanteDetalle['Debe'];
                    $comprobanteRRhhDetalle->haber = $comprobanteDetalle['Haber'];
                    $comprobanteRRhhDetalle->pais = isset($comprobanteDetalle['CentroCosto1']) ? $comprobanteDetalle['CentroCosto1'] : "";
                    $comprobanteRRhhDetalle->canal = isset($comprobanteDetalle['CentroCosto2']) ? $comprobanteDetalle['CentroCosto2'] : "";
                    $comprobanteRRhhDetalle->tienda = isset($comprobanteDetalle['CentroCosto3']) ? $comprobanteDetalle['CentroCosto3'] : "";
                    $comprobanteRRhhDetalle->save();    
                }
                
                $respuesta=array(
                    'success' => true,
                    'mensaje' => 'Los datos fueron recepcionados correctamente por Contabilidad.',
                    'comprobante' => $comprobante
                );
                ...
                $respuesta=array(
                    'success' => false,
                    'mensaje' => $datosConexionCME['mensaje'],
                    'comprobante' => $comprobante
                );
            }
    ...
    public function construirArregloCtaFinal($detalles, &$cuentasFinal, &$codigosCentroCosto, $periodo, $cuentas, $nivelActual)
    {
        $empresa = \Session::get('empresa');
        ...
        if(array_key_exists('detallesCC', $detalles)){
            ...
            if( $totalesCC['debe'] > 0 or $totalesCC['haber'] > 0 ){
                ...
                if(count($codigosCentroCosto)){
                    $eliminar = array_pop($codigosCentroCosto);
                    return false;
                }
            }
            return false;
            ...
            foreach( $detalles as $key => $detalle ){
                ...
                $resultado = $this->construirArregloCtaFinal($detalles[$key], $cuentasFinal, $codigosCentroCosto, $periodo, $cuentas, $nivelActual);
                if($resultado){
                    if(count($codigosCentroCosto)){
                        $eliminar = array_pop($codigosCentroCosto);
                    }
                    ...
                    if (count($general['cuentas'])) {
                        ...
                        if($empresa->centro_costo){
                                                                            ...
                                                                            if( $item['monto'] != 0  ) {
                                                                                ...
                                                                                if (!array_key_exists($item['nombre'], $arregloDef['detallesCC'][$item['idCuenta']['codigo']])) {
                                                                                    $arregloDef['detallesCC'][$item['idCuenta']['codigo']][$item['nombre']] = array(
                                                                                        ...
                                                                                        'haber' => 0,
                                                                                        'tipo' => $indexDetalle,
                                                                                        'cuenta' => $listaCuentas[$item['idCuenta']['codigo']]['codigo'],
                            ...
                            if(count($datosResumen['detalles'])) {
                                ...
                                $this->construirArregloCtaFinal($datosResumen['detalles'], $comprobanteFinal, $codigosCentroCosto, $general['periodo'], $listaCuentas, 1);
                            }
                            ...
                            if ($datosResumen ) {
                                ...
                                if(count($comprobanteFinal)) {
                                    foreach ($comprobanteFinal as $detalle) {
                                        ...
                                        $comprobanteDetalle->Pais = 'CHL';
                                        $comprobanteDetalle->Canal = 'ADM';
                                        $comprobanteDetalle->Tienda = 'GCIAGRAL';
                            ...
                            if (is_array($general) and is_array($detalles) and is_array($listaCuentas)) {
                                ...
                                if (count($detalles)) {
                                    foreach ($detalles as $detalle) {
                                        ...
                                        $cuentasCodigo = Funciones::array_column($listaCuentas, 'codigo', 'id');
                                        if(!$remuneraciones->cuenta($cuentasCodigo, $detalle['centroCosto'])){
                                            $errores[]= "Aporte : Remuneraciones por Pagar: Cuenta no asignada";
                                        }
                                        if(!$cotizaciones->cuenta($cuentasCodigo, $detalle['centroCosto'])){
                                            $errores[]= "Aporte : Cotizaciones por Pagar: Cuenta no asignada";
                                        }
                                        // descuentos, haberes y aportes del trabajador
                                        $arrayDetalles = array('haberes', 'descuentos', 'aportes' );
                                        foreach( $arrayDetalles as $indexDetalle ) {
                                                ...
                                                foreach ($detalle[$indexDetalle] as $item) {
                                                    if (array_key_exists('nombre', $item)
                                                        ...
                                                        if ( $item['idCuenta'] ) {
                                                            ...
                                                            $errores[]= ucwords($indexDetalle) . " : " . $item['nombre'] . " : Cuenta no asignada";
                                                        }
                                                    }
                                                }
                                            }
