    ...
    <body>    
        ...
        <table id="mes">
            <tbody>
                <tr>
                    <td colspan="21"><b>LIBRO DE REMUNERACIONES</b></td>
                </tr>
                <tr>
                    <td colspan="21"><b>MES: {{ $datos->mes }}</b></td>
                </tr>
            </tbody>
        </table>
        <table id="libro">
            <thead>
                <tr>
                    ...
                    @if($datos->conceptos['apv'])<th>APV</th>@endif        
                    ...
                    @if($datos->conceptos['total_descuentos'])<th>Otros. Desc.</th>@endif         
                    @if($datos->conceptos['otros_descuentos'])<th>Tot. Desc.</th>@endif         
            ...
            <tbody>
                @foreach($datos->liquidaciones as $dato)
                    <tr>
                        ...
                        @if($datos->conceptos['horas_extra'])<td>{{ $dato->horas_extra }}</td>@endif
                        @if($datos->conceptos['sueldo'])<td>{{ $dato->sueldo }}</td>@endif
                        @if($datos->conceptos['gratificacion'])<td>{{ $dato->gratificacion }}</td>@endif
                        @if($datos->conceptos['total_imponibles'])<td>{{ $dato->renta_imponible }}</td>@endif
